    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Space {kind === 'image' ? '(for "save to listing")' : '(optional)'}</label>
      ...
      <div>
        ...
        {kind === 'image' ? (
          <div>
            <div className="bg-white border border-gray-200 rounded-md p-3 min-h-[320px] flex items-center justify-center">
              {imgB64 ? (
                ...
                <div className="text-center text-gray-300 py-16">
                  ...
                  <p className="text-sm text-gray-400">Generated images appear here.</p>
                </div>
              )}
            </div>
            {imgB64 && (
              <div className="flex items-center gap-3 mt-3">
                <button onClick={saveToUnit} disabled={!space || saving}
                  className="flex items-center gap-1.5 text-sm font-medium bg-black text-white px-3 py-2 rounded-md hover:bg-gray-800 disabled:opacity-40">
                  {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Save to {space ? space.unitNumber : 'unit'} photos
                </button>
                <button onClick={downloadImg} className="flex items-center gap-1.5 text-sm text-gray-600 hover:text-black">
                  <Download size={14} /> Download
                </button>
              </div>
            )}
            {!space && imgB64 && <p className="text-xs text-gray-400 mt-2">Pick a space above to save this to its listing.</p>}
          ...
          <>
            ...
            {output && (
              <button onClick={handleGenerate} disabled={loading} className="mt-3 flex items-center gap-1.5 text-xs text-gray-500 hover:text-black">
