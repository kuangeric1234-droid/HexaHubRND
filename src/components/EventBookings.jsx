  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
          <div>
            <h2 className="font-bold text-gray-900">Send Event Documents</h2>
            <p className="text-xs text-gray-400 mt-0.5">Sends rundown + vendor map to selected vendors via email</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700"><X size={18} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {results ? (
            ...
            <>
              {/* Files being sent */}
              <div>
                <div className="text-xs font-medium text-gray-600 mb-2">Files included</div>
                <div className="space-y-2">
                  {EVENT_DOCS_FILES.map(f => (
                    <div key={f.key} className="flex items-center gap-3 border border-gray-200 rounded-md px-3 py-2.5">
              ...
              {/* Status filter */}
              <div>
                <div className="text-xs font-medium text-gray-600 mb-2">Send to vendors with status</div>
              ...
              {/* Recipient list */}
              <div>
                <div className="text-xs font-medium text-gray-600 mb-2">Recipients ({recipients.length})</div>
                {recipients.length === 0 ? (
                  <div className="text-xs text-gray-400 bg-gray-50 border border-gray-200 rounded-md px-3 py-3">
                  ...
                  <div className="border border-gray-200 rounded-md divide-y divide-gray-100 max-h-44 overflow-y-auto">
                    {recipients.map(b => (
                      <div key={b.id} className="flex items-center justify-between px-3 py-2">
                        <div>
                          <span className="text-sm text-gray-800 font-medium">{b.vendorName}</span>
                          {b.vendorBusiness && <span className="text-xs text-gray-400 ml-1.5">{b.vendorBusiness}</span>}
                        </div>
                        <span className="text-xs text-gray-400">{b.vendorEmail}</span>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="px-6 py-4 border-t border-gray-200 shrink-0 flex gap-3">
          <button onClick={onClose} className="flex-1 border border-gray-200 text-gray-700 py-2.5 rounded-md text-sm font-medium hover:bg-gray-50">
          ...
          {!results && (
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 bg-black text-white py-2.5 rounded-md text-sm font-semibold hover:bg-gray-800 disabled:opacity-40"
            >
              {sending ? (
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
          <div>
            <h2 className="font-bold text-gray-900">Send Email to All Vendors</h2>
            <p className="text-xs text-gray-400 mt-0.5">Opens in your email client with vendors in BCC</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700"><X size={18} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {recipients.length > 0 && (
                <button onClick={copyEmails} className="flex items-center gap-1 text-xs text-gray-400 hover:text-gray-700 underline">
