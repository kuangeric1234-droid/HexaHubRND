...
export default function PortalMessages({ tenant }) {
  ...
  async function loadMessages() {
    ...
    const unread = all.filter(m => m.sender === 'admin' && !m.readByTenant)
    for (const m of unread) {
      await supabase.from('portal_messages').upsert({
        id: m.id,
        data: { ...m, readByTenant: true },
      })
    }
  }
        ...
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender === 'tenant' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              ...
              <p className={`text-xs mt-1 ${msg.sender === 'tenant' ? 'text-gray-400' : 'text-gray-400'}`}>
                {fmt(msg.timestamp)}
              </p>
            </div>
          </div>
